...
class ClipAudio(Preprocessor):
    ...
    def parameters(cls):
        ...
        parameters.update({
            'size': NumberField(
                ...
                description="Length of audio clip in ms."
            ),
            'quantity' : NumberField(
                value_type=int, min_value=1, description="Number of clips per audiofile."
            ),
        })
        return parameters
    ...
    def configure(self):
        ...
        self.quantity = self.get_value_from_config('quantity')
